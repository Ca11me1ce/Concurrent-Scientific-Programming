// compiles with:
// g++ optimized_code.cpp -o optimized_code
        ...
        std::vector<std::vector<double> > &v_mat, std::vector<int> &i_v)
{
        ...
        double d_val;
        int i,j; 
        for (j = 0; j < v_s.size(); j +=2)
        {
                for (i = 0; i < v_s[0].size(); i+=2)
                {
                        d_val = (int)fmod(i_v[i],256); // this should return an integer
                        v_mat[i][j] = v_s[i][j]*(-cos(d_val+d_val));
                        v_mat[i+1][j+1]=v_s[i+1][j+1]*(-cos(d_val+d_val));
                }
        }
}
